...
(async () => {
    ...
    const smtp_credentials = await inquirer_1.default.prompt([
        { message: "Enter SMTP username", name: "username", type: "input", validate: (e) => {
                if (!e)
                    return "username is invalid";
                return true;
            } },
        { message: "Enter SMTP password", name: "password", type: "password", validate: (e) => {
                if (!e)
    ...
    const email_config = await inquirer_1.default.prompt([
        { message: "Enter Sender e.g MyCompany", name: "from", type: "input", validate: (e) => {
                if (!e)
                    return "from is invalid";
                return true;
            } },
        { message: "Enter Email Subject", name: "subject", type: "input", validate: (e) => {
                if (!e)
                    return "subject is invalid";
                return true;
            } },
        { message: "Enter Email Body/Content/Html", name: "content", type: "input", validate: (e) => {
                if (!e)
                    return "contect is invalid";
                return true;
            } },
    ]);
